        ...
        <q-card style="min-width: 100vw" class="translucide-pink flex justify-center">
            <q-card style="min-width: 90%" class="q-ma-md">
                <q-toolbar color="primary">
                    <template v-if="vm.map.id == 0">Ajout d'une map</template>
                    <template v-else>Editer la map</template>
                    <q-space />
                ...
                <q-card-section class="q-pt-none">
                    <q-input v-model="vm.map.nom" @keydown.enter.prevent="vm.addOrEdit()" rounded autofocus outlined
                        label="Aiza le izy e ?"></q-input>
                </q-card-section>
                <q-card-section class="flex justify-between">
                    <q-btn flat rounded color="red" v-close-popup>Annuler</q-btn>
                    <q-btn color="primary" rounded @click="vm.addOrEdit()">
                        <template v-if="vm.map.id == 0">Ajouter</template>
